...
# https://github.com/VinAIResearch/LFM/blob/50d263b0a4471384c0ab073c8f904b73c024c6c0/models/DiT.py
# Modifications: added a wrapper to align the argument order of the forward pass and to have cfg in forward pass
########################################################################################################################
########################################################################################################################
########################################################################################################################
...
class DiT(nn.Module):
    ...
    def forward_org(self, t, x, y=None, **kwargs):
        """
        Forward pass of DiT.
    ...
    def forward(self, x, t):
        """
        wrapper around original forward pass. Modifications:
        - reverse-time: integrate from 0->1 while sampling
        - cfg is always active, given by cfg_scale in init
        - class is fixed, given by init
        """
        B = x.shape[0]
        y = self.class_idx.expand(B).to(x.device)
        v_c = self.forward_org(1. - t, x, y=y)  # conditional
        v_u = self.forward_org(1. - t, x, y=None)  # unconditional (no labels)
        v = v_u + self.cfg_scale * (v_c - v_u)

        return -v  # invert vector field
